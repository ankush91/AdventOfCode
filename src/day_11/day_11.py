...
class INTCodeInterpreter():
    def __init__(self, list_op_codes, input_values):
...
def main():
    ...
    ## change input to 0 for PART-1, 1 for PART-2
    int_code_runner = INTCodeInterpreter(initial_list_op_codes[:], [1])
    direction = '^'
    op = ['color', 'direction']
    i = 0
    # change to 'B' for part-1; 'W' for part-2
    paint_cells = {(0,0): 'W'}
    x_abs, y_abs = 0, 0
    len_dir_orders = len(list_direction_order)
    ...
    while not int_code_runner.is_hault:
        ...
        curr_op = op[i % 2]
        if curr_op == 'color':
            # store color changes with key
            paint_cells[(x_abs, y_abs,)] = list_colors[output_code]
            ...
            dir_idx = (list_direction_order.index(direction) + rel_direction) % len_dir_orders
            direction = list_direction_order[dir_idx]

            # move in direction
            x_abs, y_abs = dict_directions[direction](x_abs, y_abs)
            # use color value at next position as input value; default to Black
            input_code = list_colors.index(paint_cells.get((x_abs, y_abs,), 'B'))
            int_code_runner.input_values.append(input_code)

        i += 1

    # part-1
    print('cells painted atleast once:', len(paint_cells))

    ## PART-2
    # find min_x, min_y, max_x, max_y
    min_x = min(paint_cells.items(), key=lambda x: x[0][0])[0][0]
    max_x = max(paint_cells.items(), key=lambda x: x[0][0])[0][0]
    min_y = min(paint_cells.items(), key=lambda x: x[0][1])[0][1]
    max_y = max(paint_cells.items(), key=lambda x: x[0][1])[0][1]

    span_x = abs(max_x - min_x)
    span_y = abs(max_y - min_y)

    min_x_to_orig = min_x - 0
    min_y_to_orig = min_y - 0

    print('span-x:', span_x, 'span-y:', span_y)
    print('min-x:', min_x_to_orig, 'min-y:', min_y_to_orig)

    for y in range(span_y + 1):
        str_y = ""
        for x in range(span_x + 1):
            color = paint_cells.get((x + min_x_to_orig, y + min_y_to_orig,), 'B')
            if color == 'B':
                str_y += '.'
            elif color == 'W':
                str_y += '#'
