...
def BFS(list_op_codes, simulation_speed=0.01):
    ...
    fewest_steps = 0
    loc_oxygen_system, fewest_steps_to_system = None, None
    loc_visited = set()
    ...
    # till queue is not exhausted
    while queue_directions:
        # FIFO accces to queue
        xd, yd, int_code_runner, fewest_steps = queue_directions.pop(0)

        # evaluate drone loc directions if not wall or visited
        if (xd, yd,) not in loc_visited:
            system_coordinates = directions(xd, yd, int_code_runner, fewest_steps)
        loc_visited.add((xd, yd,))

        if system_coordinates:
            loc_oxygen_system = system_coordinates
            fewest_steps_to_system = fewest_steps + 1
    ...
    print('fewest steps to oxygen system:', fewest_steps_to_system)
    ...
    return loc_oxygen_system, limits, xy_to_symbol_names


def walk_complete_map(xy_to_symbol_names, limits, loc_oxygen_system, simulation_speed=None):
    x, y = loc_oxygen_system[0], loc_oxygen_system[1]
    list_visited_at_depth = [(x, y,)]
    depth = 0

    oxygenate_x_y = lambda x, y: (
                                    ((x, y,) in xy_to_symbol_names)
                                            and 
                                    (xy_to_symbol_names[(x, y,)] == 'EMPTY')
                                )

    while list_visited_at_depth:
        frontier_expansions = []
        for xi, yi in list_visited_at_depth:
            if oxygenate_x_y(xi, yi - 1,):
                frontier_expansions.append((xi, yi - 1,))
                xy_to_symbol_names[(xi, yi - 1,)] = 'OXYGEN_SYSTEM'

            if oxygenate_x_y(xi, yi + 1,):
                frontier_expansions.append((xi, yi + 1,))
                xy_to_symbol_names[(xi, yi + 1,)] = 'OXYGEN_SYSTEM'

            if oxygenate_x_y(xi - 1, yi,):
                frontier_expansions.append((xi - 1, yi,))
                xy_to_symbol_names[(xi - 1, yi,)] = 'OXYGEN_SYSTEM'

            if oxygenate_x_y(xi + 1, yi,):
                frontier_expansions.append((xi + 1, yi,))
                xy_to_symbol_names[(xi + 1, yi,)] = 'OXYGEN_SYSTEM'
        
        list_visited_at_depth = frontier_expansions
        depth += 1
        if simulation_speed:
            time.sleep(simulation_speed)

        # draw state after oxygenation at each depth
        draw_display(None, None,
                     limits['min_x'], limits['min_y'],
                     limits['max_x'], limits['max_y'], xy_to_symbol_names)

    # oxygenation completed at prior depth
    print('number of minutes till expansion', depth - 1)
...
def main():
    ...
    ## PART-1
    print("PART-1")
    loc_oxygen_system, limits, xy_to_symbol_names = BFS(initial_list_op_codes[:], simulation_speed=0.0001)

    ## PART-2
    print("PART-2")
    walk_complete_map(xy_to_symbol_names, limits, loc_oxygen_system, simulation_speed=0.1)
